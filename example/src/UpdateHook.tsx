...
const UpdateHookPage = () => {
    ...
    useEffect(() => {
        function setup() {
            ...
            db.execute('CREATE TABLE updatehook_example (id INTEGER PRIMARY KEY, description TEXT, qty INTEGER, price REAL)');
            db.execute('INSERT INTO updatehook_example (description, qty, price) VALUES (?, ?, ?)', ['Pretzels', 1, 5.50]);
            db.execute('INSERT INTO updatehook_example (description, qty, price) VALUES (?, ?, ?)', ['Chips', 1, 2.25]);
...
function UpdateHookExample() {
    ...
    const rows = db.execute('SELECT * FROM updatehook_example').rows?._array ?? [];
    ...
    return (
        <View>
            ...
            <View>
                {rows.map((row, i) => (
                    <Text key={i}>{row.description} {row.qty} {row.price}</Text>
                ))}
            </View>
        </View>
    );
